...
#Supports temporal drift through schedule functions.
...
MEASURE_OPS: Set[str] = {"M", "MX", "MY", "MZ", "MR", "MRX", "MRY", "MRZ"}  # include all variants from decoder bench
...
def _pz_px_py(p: float, eta: float, bias_y: bool = False) -> Tuple[float, float, float]:
    ...
    # px + pz = p, pz/px = eta → px = p/(eta+1), pz = eta*p/(eta+1)
    px = p / (eta + 1.0)
    pz = p * eta / (eta + 1.0)
    py = px if bias_y else 0.0  # Optional Y channel
    
    # This prevents invalid probabilities if p is too high.
    s = px + py + pz
    if s > 1.0:
...
class BiasedCircuitNoiseSpec:
    ...
    eta: float = 20.0  # Bias ratio: pZ/pX default is 20
    ...
    idle_p: Optional[float] = None  # Override for idle errors
...
ScheduleFn = Callable[[int], Tuple[float, float]]  # tick_idx is (p_t, eta_t)
